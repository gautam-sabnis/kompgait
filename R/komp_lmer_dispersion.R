...
komp_lmer_dispersion <- function(data, CtrlStrain="C57BL/6NJ", model){	
	...
	ht.Gen.p <- Heatmap((lodGen2), row_names_gp = gpar(fontsize = 16, fontface='italic',
        ...
        grid.text((pvalGenSignif2[i,j]),x, y, gp = gpar(fontsize = 8))}, column_title=paste0(model,': Dispersion Phenotypes'))
	...
	ht.Sex.p <- Heatmap((lodSex2), row_names_gp = gpar(fontsize = 16, fontface='italic',
        ...
        grid.rect(x = x, y = y, width = width, height = height, gp = gpar(col = "grey"))
        grid.text((pvalSexSignif2[i,j]),x, y, gp = gpar(fontsize = 8))}, column_title=paste0(model,': Dispersion Phenotypes'))
